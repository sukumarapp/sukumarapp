...
joinBtn.addEventListener('click', () => {
    ...
    if (name) {
        ...
        // gameControls.style.display = 'flex'; // Removed to hide by default
        ...
        window.addEventListener('keydown', handleGameControlsToggle); // Add new listener
    }
});
...
socket.on('playerDestroyed', (data) => {
    for (let i = 0; i < 30; i++) {
        ...
        particles.push({
            x: data.x + 12, y: data.y + 12, // Centered on the 24x24 box
...
socket.on('smartBombBlast', (data) => {
    shockwaves.push({
        x: data.x + 16, y: data.y + 16, // Adjust to center of player
        startTime: Date.now(),
        duration: 500, // Animation duration in ms
        maxRadius: 300 // Max blast radius
    });
...
socket.on('gameEnded', (killSummary) => {
    ...
    // Store kill summary and redirect
...
function handleGameControlsToggle(event) {
    if (event.ctrlKey && event.key === 'm') {
        event.preventDefault(); // Prevent default browser action for Ctrl+M
...
function draw() {
    ...
    if (currentPowerUp) {
        ...
        if (!visual) {
            ...
            return; // Skip drawing to prevent crash
        }
        const time = Date.now();
    ...
    for (let id in players) {
        ...
        ctx.fillRect(player.x, player.y, 24, 24); // Smaller 24x24 player box
        ...
        if (player.activePowerUp) {
            ...
            if (cueType === 'energyShield') {
                ...
                ctx.lineWidth = 3;
                ctx.beginPath();
                ctx.arc(player.x + 12, player.y + 12, 17, 0, Math.PI * 2); // Adjusted shield size
                ctx.stroke();
            } else if (['haste', 'rapidFire', 'tripleShot', 'ricochet', 'piercingShot'].includes(cueType)) {
                ctx.strokeStyle = POWERUP_VISUALS[cueType].color;
                ctx.lineWidth = 2;
                ctx.strokeRect(player.x - 1, player.y - 1, 26, 26); // Adjusted outline size
            }
        }
        ...
        ctx.font = '12px Arial';
        ctx.textAlign = 'center';
        ctx.fillText(player.name, player.x + 12, player.y - 15); // Centered on the 24x24 box
        
        // Health bar adjusted to the new size
        ctx.fillStyle = 'red';
    ...
    bullets.forEach(bullet => {
        ...
        ctx.beginPath();
        ctx.arc(bullet.x, bullet.y, 3, 0, Math.PI * 2); // Smaller 3px radius bullet
        ctx.fill();
    });
